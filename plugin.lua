--- Credit (MIT license): https://github.com/overextended/cfxlua-vscode/blob/91c5fa4666b0e0e37fecba315c7134fdfd213a3e/Lua/plugin.lua
...
function OnSetText(uri, text)
	...
	for comment, vars, inPos, afterInPos, tablePos, tableName, finishPos in
		str_gmatch(text, "(%-?%-?)([_%w, ]*)%s+()in()[     ]+()([_%w]*%s-%(?.-%)?)()")
	do
		if comment == "" then
